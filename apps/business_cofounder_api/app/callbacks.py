...
def send_canvas_callback(
    ...
    Args:
        ...
        partner_query: Partner search query generated by expert (optional)
        partner_search_results: List of partner search results with proposals (optional)
    """
    if not callback_url:
        return
...
async def fetch_and_send_canvas_callback(
    ...
    try:
        ...
        if analysis is not None:
            ...
            partner_search_results = analysis.get("partner_search_results")
            ...
            if canvas is not None:
                ...
                _logger.info(
                    "[CanvasCallback] Sending canvas update from analysis dict (session_id=%s, round=%d, has_summary=%s, has_partner_query=%s, has_partner_results=%s)",
                    session_id,
                    current_round,
                    canvas_update_summary is not None,
                    partner_query is not None,
        ...
        if current_state_snapshot and hasattr(current_state_snapshot, "values"):
            ...
            if canvas is not None:
                ...
                _logger.info(
                    "[CanvasCallback] Sending canvas update from state (session_id=%s, round=%d, has_summary=%s, has_partner_query=%s, has_partner_results=%s)",
                    session_id,
                    current_round,
                    canvas_update_summary is not None,
                    partner_query is not None,
                                ...
                                if should_trigger_expert(result_state):
                                    ...
                                    try:
                                        analysis = await asyncio.wait_for(
                                            ...
                                            timeout=180.0,  # 180 second timeout for the entire expert sync
                                        )
                                        ...
                                        _logger.error("[DualAgent] Expert sync timed out after 180s (async, callback)")
                                    except Exception as e:
