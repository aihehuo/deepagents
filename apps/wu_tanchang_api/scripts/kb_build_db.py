...
def init_db(db_path: Path) -> sqlite3.Connection:
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_keywords_kw    ON note_keywords(keyword);"
    )
...
def validate_db_constraints(keyword_list: list[str], insight_list: list[str]) -> None:
    ...
    for ins in insight_list:
        ...
        if not (10 <= han_count <= 50):
            raise ValueError(
                f"Insight '{ins}' must contain 10-50 Chinese characters (got {han_count})"
            )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Build/update Wu Tanchang SQLite database."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--index-json",
        type=str,
        default="apps/wu_tanchang_api/workspace/kb/index.json",
    ...
    parser.add_argument(
        ...
        help="Path to extracted JSON files",
    )
    ...
    for item in index_data:
        ...
        if not raw_file_path.exists():
            logger.warning(
                "Raw file not found at %s for note %s. Skipping...",
                raw_file_path,
                note_id,
            )
            continue
        ...
        if not ext_file.exists():
            logger.warning(
                "Extracted metadata file %s does not exist. Please run kb_extract.py first.",
                ext_file,
            )
            continue
        ...
        try:
            ...
            logger.error(
                "Data validation failed for note %s: %s. Skipping SQLite update.",
                note_id,
                e,
            )
            continue
    ...
    conn.close()
    logger.info(
        "SQLite database build complete. Inserted/Updated: %d, Skipped: %d",
        inserted_notes,
        skipped_notes,
    )
