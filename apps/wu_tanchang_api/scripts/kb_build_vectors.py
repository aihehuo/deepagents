...
def get_embeddings(texts: list[str], api_key: str, model: str = "text-embedding-v3") -> list[list[float]]:
    ...
    for i in range(0, len(texts), batch_size):
        ...
        if resp.status_code != 200:
            raise ValueError(f"DashScope embedding failed: code={resp.status_code}, message={resp.message}")
...
def main() -> None:
    ...
    vector_env_dir = os.environ.get("WU_KB_VECTOR_DIR", "apps/wu_tanchang_api/workspace/kb/chroma")
    embedding_model = os.environ.get("WU_KB_EMBEDDING_MODEL", "text-embedding-v3")
    
    parser = argparse.ArgumentParser(description="Generate embeddings and build ChromaDB vectors.")
    parser.add_argument("--db-path", type=str, default=db_env_path, help="Path to SQLite database")
    parser.add_argument("--vector-dir", type=str, default=vector_env_dir, help="Directory for ChromaDB storage")
    parser.add_argument("--model", type=str, default=embedding_model, help="DashScope embedding model name")
    parser.add_argument("--force", action="store_true", help="Force rebuilding all vectors")
    ...
    if not db_path.exists():
        logger.error("SQLite database not found at %s. Please run kb_build_db.py first.", db_path)
        sys.exit(1)
    ...
    # Query all notes
    cursor.execute("SELECT id, title, series, brand, city, categories, topics, content_sha FROM notes;")
    ...
    for note in notes:
        ...
        cursor.execute("SELECT keyword FROM note_keywords WHERE note_id = ? ORDER BY position;", (note_id,))
        keywords = [row["keyword"] for row in cursor.fetchall()]
        
        cursor.execute("SELECT insight FROM note_insights WHERE note_id = ? ORDER BY position;", (note_id,))
    ...
    conn.close()
    logger.info("ChromaDB build complete. Updated/Rebuilt: %d, Skipped: %d", updated_notes, skipped_notes)
