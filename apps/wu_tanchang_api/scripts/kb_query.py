...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Query the Wu Tanchang KB using semantic search."
    )
    parser.add_argument("query", type=str, help="Semantic search query")
    parser.add_argument("--k", type=int, default=5, help="Number of results to return")
    parser.add_argument(
        "--series", type=str, default=None, help="Filter by series (e.g. 商业探店笔记)"
    )
    parser.add_argument(
        "--categories",
        type=str,
    ...
    print(
        f"Querying: '{args.query}' (k={args.k}, series={series_list}, categories={cats_list})..."
    )
