...
def create_cli_agent(
    ...
    *,
    ...
    # Checkpointer:
    # - Use passed-in checkpointer if provided
    # - Else Business Co-Founder preset persists checkpoints to disk for conversation resume
    # - Else in-memory only
    if checkpointer is not None:
        final_checkpointer = checkpointer
    elif preset == "business_cofounder":
        ckpt_path = settings.get_agent_dir(assistant_id) / "conversation_checkpoint.pkl"
        final_checkpointer = _DiskBackedInMemorySaver(file_path=ckpt_path)
    else:
