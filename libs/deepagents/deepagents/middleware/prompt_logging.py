...
class PromptLoggingMiddleware(AgentMiddleware):
    ...
    def _log_prompt_stack(self, request: ModelRequest) -> None:
        ...
        """
        _logger.info("=" * 100)
        _logger.info("[PromptLoggingMiddleware] ===== COMPLETE PROMPT STACK (RIGHT BEFORE LLM CALL) =====")
        _logger.info("=" * 100)
        ...
        if hasattr(request, "system_prompt"):
            ...
            if system_prompt:
                _logger.info("[PromptLoggingMiddleware] SYSTEM PROMPT (VERIFIED - EXISTS):")
                _logger.info("-" * 100)
                _logger.info("%s", system_prompt)
                _logger.info("-" * 100)
                _logger.info("[PromptLoggingMiddleware] System prompt length: %d characters", len(system_prompt))
                ...
                if "CRITICAL INSTRUCTION" in system_prompt or "PRIMARY DIRECTIVE" in system_prompt:
                    _logger.info("[PromptLoggingMiddleware] ✓ EXPERT GUIDANCE DETECTED in system prompt!")
                elif "Strategic Guidance" in system_prompt:
                    _logger.info("[PromptLoggingMiddleware] ⚠ Strategic Guidance section found but may not be strong enough")
                else:
        ...
        if hasattr(request, "system_message"):
            ...
            if system_message:
                ...
                _logger.info("[PromptLoggingMiddleware] SYSTEM MESSAGE (alternative):")
                _logger.info("-" * 100)
                _logger.info("%s", content[:500] if len(str(content)) > 500 else content)
                _logger.info("-" * 100)
        ...
        if hasattr(request, "messages") and request.messages:
            _logger.info("[PromptLoggingMiddleware] MESSAGES (%d total):", len(request.messages))
            _logger.info("-" * 100)
            for i, msg in enumerate(request.messages, 1):
                ...
                _logger.info("[PromptLoggingMiddleware] Message %d (%s):", i, msg_type)
                _logger.info("%s", content_preview)
                _logger.info("")
        else:
            _logger.info("[PromptLoggingMiddleware] MESSAGES: (none or not accessible)")
        ...
        if hasattr(request, "tools") and request.tools:
            ...
            _logger.info("[PromptLoggingMiddleware] AVAILABLE TOOLS (%d total): %s", len(request.tools), ", ".join(tool_names))
        else:
            _logger.info("[PromptLoggingMiddleware] AVAILABLE TOOLS: (none)")
        ...
        if hasattr(request, "model") and request.model:
            ...
            _logger.info("[PromptLoggingMiddleware] MODEL: %s", model_info)
        else:
            _logger.info("[PromptLoggingMiddleware] MODEL: (not accessible from request)")
        
        _logger.info("=" * 100)
        _logger.info("[PromptLoggingMiddleware] ===== END OF PROMPT STACK =====")
        _logger.info("=" * 100)
        _logger.info("")
