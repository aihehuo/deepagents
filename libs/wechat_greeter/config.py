...
def apply_tail_and_truncate(
    ...
    """Ensure raw_reply ends with tail exactly once (if tail is set).

    Deduplicates/removes existing tail (or common variants like Chinese/English comma or newlines)
    to prevent duplicate tails in output reply.
    ...
    return (cleaned + tail) if tail else cleaned
