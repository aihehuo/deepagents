...
class DirectoryOnlyBackend:
    """Backend wrapper that forces all writes/edits into the test file's directory.
    
    This ensures the agent can only write/edit files in the same directory as the test script,
    ...
    def __init__(self, *, backend: BackendProtocol, test_dir: str | Path) -> None:
        """Initialize the test directory-only backend.
        
        Args:
            backend: The underlying backend to delegate to.
            test_dir: The test file's directory where writes/edits are allowed.
        """
