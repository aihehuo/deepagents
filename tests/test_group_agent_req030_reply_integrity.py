...
def test_finalize_empty_match_without_invite_keeps_clarifying_reply() -> None:
    """Empty match + no invite card must not glue「暂未找到」under clarifying Q&A."""
    profile = profile_from_flat(
        user_id="1",
        group_id="763",
    ...
    clarifying = (
        "你刚说「最需要的是有教研经验的合伙人」——这和已有 need 高度一致，"
        "但需补一个关键细节：你希望这位合伙人具体参与哪类动作？"
    )

    result = finalize_user_visible_reply(
        original_reply=clarifying,
        profile=profile,
        profile_persisted=True,
        match_status="empty",
        candidate_count=0,
    ...
    assert "暂未找到足够明确的本群人选" not in result
