...
class TestTodoListWithRealLLM:
    ...
    def test_agent_creates_and_executes_all_todos(self) -> None:
        ...
        Uses a real LLM configured via environment variables. Provider selection:
        - BC_API_PROVIDER=qwen -> Qwen (QWEN_* env vars)
        - otherwise -> Anthropic-compatible / DeepSeek (ANTHROPIC_* env vars)
        """
...
CRITICAL:
- After executing the FINAL task, you MUST call write_todos one last time to mark it as completed (no in_progress tasks left).
...
Please create a todo list and execute all the tasks."""
        ...
        #
        # Some providers may occasionally forget the final write_todos update and leave
        # the last todo as in_progress even though execute_task ran successfully.
        # We treat a todo as done if either:
        # - status == completed, OR
        # - we can prove execute_task was called for that exact todo content.
        executed_tasks: set[str] = set()
        for msg in result.get("messages", []):
            if msg.type == "tool" and getattr(msg, "name", "") == "execute_task":
                content = str(getattr(msg, "content", "")).strip()
                marker = "✅ Successfully completed task:"
                if marker in content:
                    executed_tasks.add(content.split(marker, 1)[1].strip())

        assert executed_tasks, "Expected at least one execute_task tool response"

        incomplete = []
        for todo in result["todos"]:
            content = todo.get("content", "")
            status = todo.get("status", "")
            if status != "completed" and content not in executed_tasks:
                incomplete.append(todo)

        assert not incomplete, (
            "All todos should be completed or executed. "
            f"Incomplete todos: {incomplete}. Executed tasks: {sorted(executed_tasks)}"
        )
